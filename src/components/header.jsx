...
const Header = () => {
  ...
  const {user,logOut}=useContext(AuthContext);
   ...
   const handleSignOut=()=>{
    ...
    return (
<div>
 <div className={`navbar ${dark?"bg-blue-900 text-black":"bg-blue-500 text-white" }`}>
  <div className="navbar-start">
    <div className="dropdown">
      <label tabIndex={0} className="btn btn-ghost lg:hidden">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
      </label>
              <ul tabIndex={0} className={`menu menu-compact  dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-52 ${dark ? "bg-blue-900 text-black" : "bg-blue-500 text-white"} `}>
     ...
     <li><input type="checkbox" className="toggle toggle-md" onClick={handleDark} /></li>
      </ul>
    </div>
...
<label className="swap swap-rotate ml-12 ">
  ...
  <svg className="swap-off fill-gray-900 stroke-white w-10 h-10" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"/></svg>
